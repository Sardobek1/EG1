...
import com.sardorbek.eg1.databinding.ActivityEg2Binding

class Eg2 : AppCompatActivity() {
    var isResendable = false
    private lateinit var binding : ActivityEg2Binding
    override fun onCreate(savedInstanceState: Bundle?) {

        binding = ActivityEg2Binding.inflate(layoutInflater)
    ...
    private fun startTimer() {
        ...
        val timer = object: CountDownTimer(120000, 1000) {
            override fun onTick(millisUntilFinished: Long) {
                ...
                binding.timeclock.text = timeConversion(seconds.toInt())
            }

            @RequiresApi(Build.VERSION_CODES.M)
            override fun onFinish() {
                binding.timeclock.text = "Resend"
                isResendable = true
                binding.timeclock.setTextColor(getColor(R.color.purple_500))
            }
        }
        timer.start()
